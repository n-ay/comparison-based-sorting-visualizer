...
def quick_sort(data, start, end, drawData, timeTick):
    if start < end:
        ...
        drawData(data, [PINK if x >= start and x < pivot_position else YELLOW if x == pivot_position
                        else BLUE if x > pivot_position and x <=end else PURPLE for x in range(len(data))])
        time.sleep(timeTick)
